...
export function HDRImage() {
	...
	res.toHDRBlob = function (cb, m, q) {
		...
		function createProgram(gl, vertexShaderSource, fragmentShaderSource) {
			...
			gl.attachShader(
				program,
				(vs = createShader(gl, vertexShaderSource, gl.VERTEX_SHADER)),
			);
			gl.attachShader(
				program,
				(fs = createShader(gl, fragmentShaderSource, gl.FRAGMENT_SHADER)),
			);
...
function floatToLDR(buffer, exposure, gamma, res) {
	...
	for (var i = 0; i < l; i++) {
		...
		res[i * 4 + 1] =
			255 * Math.pow(buffer[i * 3 + 1] * exposure, one_over_gamma);
		res[i * 4 + 2] =
			255 * Math.pow(buffer[i * 3 + 2] * exposure, one_over_gamma);
		res[i * 4 + 3] = 255;
	}
	return res;
}
